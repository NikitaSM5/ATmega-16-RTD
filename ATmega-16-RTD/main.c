...
// ----------------------------------------------------------------------------
// Ключевая идея рефакторинга: никаких обращений к шине SPI и «долгих» операций
// внутри прерываний. Прерывания только выставляют флаги?события, а главный
// цикл обрабатывает их последовательно.
// ----------------------------------------------------------------------------
...
static void scroll_update(void)
{
	...
	if (now - scroll_t0 >= SCROLL_STEP_MS) {
		scroll_t0 = now;
		if (++scroll_ofs > scroll_len - 16) { scroll_ofs = 0; scroll_t0 = now; }
		lcd_mov_cursor(16);
...
ISR(TIMER0_OVF_vect)
{
	if (flags.uart_dump) PORTA = 0x00;
	else sevseg_display_process(disp_digits);
	...
	if (pressed_edge & BTN_FWD_MASK) {
		...
		last_fwd_ms = now;
		if (fwd_clicks == 2) { nav_end_event = 1; fwd_clicks = 0; }
		flags.nav_fwd = 1; fwd_hold_cnt = 0; fwd_rep_cnt = HOLD_INITIAL_DELAY_MS;
	}
	if (!(cur & BTN_FWD_MASK)) {
	...
	if (pressed_edge & BTN_BWD_MASK) {
		...
		last_bwd_ms = now;
		if (bwd_clicks == 2) { nav_start_event = 1; bwd_clicks = 0; }
		flags.nav_bwd = 1; bwd_hold_cnt = 0; bwd_rep_cnt = HOLD_INITIAL_DELAY_MS;
	}
	if (!(cur & BTN_BWD_MASK)) {
...
int main(void)
{
	...
	while (1) {
		...
		if (flags.one_sec) {
			...
			lcd_mov_cursor(0); lcd_disp_str((uint8_t *)tstr);
			lcd_mov_cursor(10);
			lcd_disp_str((uint8_t *)"#");
		...
		if (!flags.show_history) {
			if (nav_end_event && !flags.btn_lock) {
				nav_end_event = 0; flags.btn_lock = 1; sd_iter_to_end(); go_to_log_end(); flags.btn_lock = 0;
			}
			if (nav_start_event && !flags.btn_lock) {
				nav_start_event = 0; flags.btn_lock = 1; sd_iter_reset(); go_to_log_start(); flags.nav_bwd = 0; flags.btn_lock = 0;
			}
			if (flags.nav_fwd && !flags.btn_lock) {
				flags.nav_fwd = 0;
				if (nav_pos < meas_no && sd_read_line_at(++nav_pos, linebuf, sizeof linebuf) == 0) {
					char *e = strchr(linebuf, '\n'); if (e) *e = 0;
					lcd_mov_cursor(16); lcd_disp_str((uint8_t *)linebuf); lcd_disp_str((uint8_t *)"    ");
					strncpy(scroll_buf, linebuf, sizeof scroll_buf - 1);
					scroll_buf[sizeof scroll_buf - 1] = 0;
					scroll_len = strlen(scroll_buf); scroll_ofs = 0; scroll_t0 = ms_ticks;
				}
			}
			if (flags.nav_bwd && !flags.btn_lock) {
				flags.nav_bwd = 0;
				if (nav_pos > 1 && sd_read_line_at(--nav_pos, linebuf, sizeof linebuf) == 0) {
					char *e = strchr(linebuf, '\n'); if (e) *e = 0;
					lcd_mov_cursor(16); lcd_disp_str((uint8_t *)linebuf); lcd_disp_str((uint8_t *)"    ");
					strncpy(scroll_buf, linebuf, sizeof scroll_buf - 1);
					scroll_buf[sizeof scroll_buf - 1] = 0;
		...
		if (flags.uart_dump && !flags.btn_lock) {
			flags.btn_lock = 1;
			sd_flush();
			lcd_send_cmd(1 << LCD_CLR); _delay_ms(2);
			uart_dump_log();
			sd_flush(); sd_erase_sector(0); sd_clear_log(1);
			meas_no = 0; nav_pos = 0; last_sent_entry = 0;
			lcd_send_cmd(1 << LCD_CLR); _delay_ms(2);
			lcd_mov_cursor(10); lcd_disp_str((uint8_t *)"#0   ");
			flags.btn_lock = 0; flags.uart_dump = 0;
		}
